    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('especialidades.edit', $especialidad->id) }}" class="btn btn-warning">Modificar</a>
                    <form action="{{ route('especialidades.destroy', $especialidad->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger" onclick="return confirm('¿Seguro que quieres eliminar esta especialidad?')">Borrar</button>
                    </form>
                </td>
            </tr>
            @endforeach
